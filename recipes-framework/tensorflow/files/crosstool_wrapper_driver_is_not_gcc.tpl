...
for val in VALS:
    if val not in os.environ:
        print(val, 'needs to be defined in the environment')
        sys.exit(1)

# Template values set by cuda_autoconf.
CPU_COMPILER = (os.environ['GCC_HOST_COMPILER_PATH'])
GCC_HOST_COMPILER_PATH = (os.environ['GCC_HOST_COMPILER_PATH'])
...
def main():
  ...
  if ofile and ofile[0].endswith('libtensorflow_framework.so'):
    cpu_compiler_flags += [
        '-Wl,-rpath,'+os.getcwd()+'/bazel-out/arm-opt/genfiles/external/local_config_cuda/cuda/cuda/lib',
        '-Wl,-rpath,%{cuda_toolkit_path}/lib/stubs',
        '-Wl,-rpath,%{cuda_toolkit_path}/lib',
    ]
